...
class ListaProductosProveedor(ListView):
    model = Producto
    paginate_by = 100
    ordering = "-marca"
    template_name = "buscador.html"
    ...
    def get_queryset(self):
        proveedor_q = self.request.GET.get("prov")
        result_delete = Producto.objects.filter(fecha__lt=date.today()).delete()
        # print(result_delete)
        return Producto.objects.filter(proveedor=proveedor_q).order_by('-marca', '-precio_dolares')
    
    def get_context_data(self, **kwargs):
        context = super(ListaProductosProveedor, self).get_context_data(**kwargs)
